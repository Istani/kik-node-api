...
class ImageManager {
  ...
  async uploadImg(imgPath) {
    ...
    const previewBlockhash = blockhashData(
      { data: raw.data, height: raw.info.height, width: raw.info.width },
      16,
      2
    );
    ...
    const headers = {
      ...
      'Content-Length': size,
      'User-Agent': `Kik/${config(15.60).kikVersionInfo.version} (Android 10.0.0) Content`,
      'x-kik-jid': this.senderJid,
      'x-kik-password': cryptoUtils.generatePasskey(
        this.username,
        this.password
      ),
      'x-kik-verification': cryptoUtils.generateImageVerification(
        contentId,
        this.appId
      ),
  ...
  parseAppData(data) {
    ...
    if (
      data.find('app-name').text == 'Gallery' ||
      data.find('app-name').text == 'Camera'
    ) {
      ...
      file_name = data
        .find('uris')
        .find('uri')
        .attrs['file-content-type'].replace('/', '.');
      ...
      for (let i in tmp_elements) {
        ...
        if (element.find('key').text == 'sticker_url') {
          // Probably a Array.Find would work too
      ...
      for (let i in tmp_elements) {
        let element = tmp_elements[i];
        if (element.find('key').text == 'jsonData') {
          // Probably a Array.Find would work too
