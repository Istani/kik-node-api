  ...
  if (xmlns === 'jabber:iq:register') {
    ...
    if (error && error.attrs.code === '404') {
      ...
      users = data.findAll('item').map((user) => ({
        jid: user.attrs.jid,
        username:
          user.find('username').text === 'Username unavailable'
            ? null
    ...
    if (method.startsWith('GetUsers')) {
      ...
      const parsedGroups = protobuf
        .lookupType('FindGroupsResponse')
        .decode(Buffer.from(data.find('body').text, 'base64'))
        .match.map(({ jid, displayData, groupJoinToken, memberCount }) => {
          ...
          return {
            ...
            joinToken: base64JoinToken.endsWith('=')
              ? base64JoinToken.slice(0, base64JoinToken.indexOf('='))
              : base64JoinToken,
            memberCount,
          };
        });
