...
cryptoUtils.generatePasskey = (username, password) => {
  ...
  return crypto
    .pbkdf2Sync(sha1Password, salt, 8192, 16, 'sha1')
    .toString('hex');
};
cryptoUtils.generateUUID = () => {
  ...
  let j = msb
    .and(-16777216)
    .shiftRight(22)
    .xor(msb.and(16711680).shiftRight(16))
    .xor(msb.and(65280).shiftRight(8));
  ...
  for (let i = 0; i < 6; i++) {
    ...
    lsb = lsb
      ...
      .or(UUIDSubFunc(j, i).shiftLeft(k + 2));
  }
  let final =
    converter.decToHex(msb.toString(), { prefix: false }) +
    converter.decToHex(lsb.toString(), { prefix: false });
  return `${final.slice(0, 8)}-${final.slice(8, 12)}-${final.slice(
    12,
    16
  )}-${final.slice(16, 20)}-${final.slice(20)}`;
};
...
cryptoUtils.generateCV = (versionInfo, timestamp, jid) => {
  ...
  let keySource =
    'hello' +
    Buffer.from(apkSignatureHex, 'hex').toString('binary') +
    versionInfo.version +
    versionInfo.sha1Digest +
    'bar';
  let hmacKey = crypto
    .createHash('sha1')
    .update(keySource, 'ascii')
    .digest('base64');
...
cryptoUtils.generateSignature = (kikVersion, timestamp, jid, sid) => {
  ...
  let data = crypto
    .createHash('sha256')
    .update(`${jid}:${kikVersion}:${timestamp}:${sid}`)
    .digest('hex');
  return crypto
    .createSign('RSA-SHA1')
    .update(data)
...
function payloadHash(map) {
  ...
  let arr = [
    hashSubfunction(0, string1),
    hashSubfunction(1, string1),
    hashSubfunction(2, string1),
    hashSubfunction(0, string2),
    hashSubfunction(1, string2),
    hashSubfunction(2, string2),
  ];
...
function hashSubfunction(hashId, bytes) {
  ...
  for (let i = 0; i < buffer.length; i += 4) {
    j ^=
      (byteToSignedInt(buffer[i + 3]) << 24) |
      (byteToSignedInt(buffer[i + 2]) << 16) |
      (byteToSignedInt(buffer[i + 1]) << 8) |
      byteToSignedInt(buffer[i]);
  }
  return j;
}
