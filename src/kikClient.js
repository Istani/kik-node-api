...
module.exports = class KikClient extends EventEmitter {
  ...
  authenticate(usernameOrEmail, password) {
    if ((usernameOrEmail && !password) || (password && !usernameOrEmail)) {
      throw new Error(
        'Username/email and password must be provided together, check your authenticate call'
      );
    }
    this.connect(() => {
  ...
  setNode(node, username) {
    ...
    sessionUtils.saveSession(
      username || this.createAccountParams.username,
      this.session
    );
  ...
  authRequest(username, password) {
    ...
    this.connection.sendXmlFromJs(
      auth(username, password, this.session.node, this.session.deviceID),
      true
    );
  }
  ...
  getUserInfo(usernamesOrJids, useXiphias, callback) {
    this.logger.log(
      'info',
      `Requesting user info with Xiphias = ${useXiphias} for ${usernamesOrJids}`
    );
